    ...
    public class SecurityAPIController : ApiController
    {
        ...
        public HttpResponseMessage Register([FromBody]JObject account)
        {
            try
            {
                ...
                if (user == null)
                {
                    return new Response(1001, "学工号不存在，请联系教师或管理员更新名单").Convert();
                }

                if (user.is_accept != null && user.is_accept == true)
                    return new Response(1001, "已激活").Convert();
                if (user.is_accept != null && user.name != name)
                {
                    return new Response(1001, "姓名填写错误").Convert();
                }
                ...
                if (user.email != null)
                {
                    ...
                    LogUtil.Log(Request, "激活", user.id, user.id, user.role, "", user.email, DateTime.Now.ToString());
        ...
        public HttpResponseMessage Login([FromBody]JObject account)
        {
            try
            {
                ...
                User user = UserDao.GetUserById(id.ToUpper());
                if(user == null)
                {
                ...
                if (pwd.Equals(user.passwd))
                {
                    ...
                    if (user.is_accept != true)
                        return new Response(2002, "账号未激活，请点击主页“激活账户”填写个人信息激活账号").Convert();
                    string uuid = System.Guid.NewGuid().ToString();
                    redis.Set(uuid, user.id, 60);
                    redis.Set(user.id, uuid, 60);
                    Dictionary<string, string> retData = new Dictionary<string, string>
                    {
                        { "authorization", uuid },
        ...
        [Route("security/getUserRole"), HttpGet]
        public HttpResponseMessage GetUserRole()
        {
            try
            {
                ...
                return new Response(1001, "成功获取权限", user.role).Convert();
            }
            catch (Exception e)
            {
                ErrorLogUtil.WriteLogToFile(e, Request);
        ...
        public HttpResponseMessage ResetPasswordRequest([FromBody]JObject account)
        {
            ...
            try
            {
                ...
                User user = UserDao.GetUserById(id);
                if (user == null)
                    return new Response(1001, "请求成功").Convert();
                if (user.email == null || user.email.Length < 1 || user.is_accept != true)
                    return new Response(1002, "未激活或未添加邮箱地址，请先激活或联系管理员重置密码").Convert();
        ...
        public HttpResponseMessage ActivateAccountVerify([FromBody]JObject accountInfo)
        {
            try
            {
                ...
                if (!(id.ToUpper().Equals(redis.Get<string>(auth).ToUpper())))
                {
