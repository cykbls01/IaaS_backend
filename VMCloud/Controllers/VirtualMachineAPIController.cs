...
namespace VMCloud.Controllers
{
    ...
    public class VirtualMachineAPIController : ApiController
    {
        ...
        [Route("vm/syncNode"), HttpPost]
        public HttpResponseMessage SyncNode([FromBody]JObject createInfo)
        {
            try
            {
                string id = ConfigurationManager.AppSettings["K8sUser"];
                var jsonParams = HttpUtil.Deserialize(createInfo);
                string sign = jsonParams.Authorization;
                string signature = HttpUtil.GetAuthorization(Request);
                if (signature == null)
                {
                    return new Response(2001, "未登录账户").Convert();
                ...
                string nodeName = jsonParams.NodeName;
                string isReady = "false";
                VMConfig vc = VMDao.GetVMByName(nodeName);
                if (vc.Status.IPAddress.StartsWith("10") == true)
                {
                    isReady = "true";
                }
                
                return new Response(1001, "检查主机状态成功",isReady).Convert();
            }
            catch (Exception e)
            {
                ErrorLogUtil.WriteLogToFile(e, Request);
        ...
        private Boolean CheckNodeHost(string host, List<VMConfig> vms)
        {
            foreach (VMConfig config in vms)
            {
                if (config.Status.IPAddress.Equals(host))
                    return false;
            }
            return true;
        }
        ...
        [Route("vm/createClusterNode"), HttpPost]
        public HttpResponseMessage CreateNode([FromBody]JObject createInfo)
        {
            try
            {
                // 权限校验
                ...
                List<VMConfig> vms = VMDao.GetAllVMS();

                // 封装参数
                string userId = id;
                VMConfig config = new VMConfig();
                config.TemplateName = ConfigurationManager.AppSettings["K8sTemplate"];
                config.CPU = int.Parse(ConfigurationManager.AppSettings["K8sCpu"]);
                config.Memory = int.Parse(ConfigurationManager.AppSettings["K8sMemory"]) * 1024;
                config.Disk = int.Parse(ConfigurationManager.AppSettings["K8sStorage"]) * 1024;
                config.Name = ConfigurationManager.AppSettings["K8sNodePrefix"] + NodeDao.GetNumber().ToString();
                ...
                node.user_name = "root";
                node.password = "&ruanjianxueyuan2021";
                node.host = "10.251.252." + (NodeDao.GetNumber()+1).ToString();
                int vernier = 2;
                while (true)
                {
                    if (CheckNodeHost(node.host, vms))
                        break;
                    else
                    {
                        node.host = "10.251.252." + (NodeDao.GetNumber() + vernier).ToString();
                        vernier++;
                    }
                }

                NodeDao.Add(node);
                createNodeVM(config, vm, userId);
                return new Response(1001, "创建集群节点成功", config.Name).Convert();
            }
            catch (Exception e)
            {
                ErrorLogUtil.WriteLogToFile(e, Request);
        ...
        /// 创建一个集群虚拟机 函数 -by zzw
        /// </summary>
        public async void createNodeVM(VMConfig vm, VirtualMachine vminfo, string sendTo = "")
        {
            string res = "";
            await Task.Run(() =>
            {
                ...
                {
                    LogUtil.Log(Request, "新建集群节点", vminfo.vm_name, vminfo.user_id, UserDao.GetUserById(vminfo.user_id).role, "success", res);
                }
                else
                {
                    LogUtil.Log(Request, "新建集群节点", vminfo.vm_name, vminfo.user_id, UserDao.GetUserById(vminfo.user_id).role, "fail", res);
                }
            });
        }
