    ...
    public class TeacherAPIController : ApiController
    {
        ...
        public HttpResponseMessage DownloadExpResource()
        {
            try
            {
                ...
                Experiment exp = ExperimentDao.GetExperimentById(expId, true);
                if (exp == null)
                ...
                foreach (var assignment in assignments)
                {
                    ...
                    switch(rename)
                    {
                        ...
                        default:
                            ...
                            break;
                    }//TODO: 文件/实验命名中不得包含：\ / : * ? " < > |以及空格，或打包时替换掉
        ...
        public HttpResponseMessage AddAstToCourse([FromBody]JObject addInfo)
        {
            try
            {
                ...
                User user = UserDao.GetUserById(id);
                Course course = CourseDao.GetCourseInfoById(courseId);
                List<Assistant> astList = CourseDao.GetAssistantsByCourseId(courseId);
                if (user.id == course.teacher_id || (user.role == 2 && astList.Where(a => a.student_id == user.id).Count() != 0))
                {
                    if (astList.Where(a => a.student_id.Equals(stuid)).Count() > 0)
                        return new Response(1002, "无需重复添加").Convert();
