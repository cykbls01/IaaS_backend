...
namespace VMCloud.Controllers
{
    ...
    public class ExperimentAPIController: ApiController
    {
        ...
        /// <summary>
        /// 根据用户id获取管理课程id，供其他平台使用
        /// </summary>
        /// <returns></returns>
        [Route("user/getCoursesByUser"), HttpGet]
        public HttpResponseMessage getCoursesByUser()
        {
            try
            {
                string signature = HttpUtil.GetAuthorization(Request);
                ...
                else if (signature == null || !redis.IsSet(signature))
                {
                    return new Response(2001, "未登录账户").Convert();
                }
                if (signature.Length > 12)
                    id = redis.Get<string>(signature);
                User user = UserDao.GetUserById(id);
                List<Course> clist = CourseDao.GetCoursesByProfessorId(id);
                List<int> cidList = new List<int>();
                foreach(Course c in clist)
                {
                    cidList.Add(c.id);
                }
                List<Assistant> alist = CourseDao.GetAssistantsByStuId(id);
                foreach (Assistant a in alist)
                {
                    cidList.Add(a.course_id);
                }
                return new Response(1001, "获取成功", cidList).Convert();
            }
            catch (Exception e)
            {
                ErrorLogUtil.WriteLogToFile(e, Request);
