    ...
    public class ExperimentDao
    {
        ...
        public static List<expReturn> GetExpRet(List<Experiment> es)
        {
            ...
            foreach (Experiment e in es)
            {
                if (e.resource != null)
                {
                    File expFile = AssignmentDao.GetFileById(e.resource);
                    e.resource = expFile == null ? e.resource : expFile.name;
                }
        ...
        public static string GetFileResource(Experiment exp)
        {
            if (exp == null || exp.resource == null)
                return null;
            if(!Regex.IsMatch(exp.resource, "^\\w{8}(-\\w{4}){3}-\\w{12}$"))
                return exp.resource;
            File expFile = AssignmentDao.GetFileById(exp.resource);
            return expFile == null ? exp.resource : expFile.name;
        }
        ...
        /// <returns>experiment对象|null</returns>
        public static Experiment GetExperimentById(int experimentId, bool needResourceId = false)
        {
            using (var dbContext = new DataModels())
            {
                Experiment experiment = dbContext.Experiments.Find(experimentId);
                if(!needResourceId) experiment.resource = GetFileResource(experiment);
                return experiment;
            }
        }
        ...
        public static List<Experiment> GetExperimentByCourseIdAndTermId(int courseId, int termId)
        {
            using (var dbContext = new DataModels())
            {
                ...
                foreach (var experiment in query)
                {
                    experiment.resource = GetFileResource(experiment);
                }
