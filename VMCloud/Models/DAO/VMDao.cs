...
namespace VMCloud.Models.DAO
{
    public static class VMDao
    {
        ...
        public static string ChangeApplyUseTime(int recordId, string dueTime)
        {
            ...
            {
                Apply_record edit_record = context.Apply_record.Find(recordId);
                if (edit_record == null || edit_record.status != 1)
                    throw new Exception("record not found");
                VirtualMachine vm = context.VirtualMachines.Find(edit_record.vm_name);
                if (vm == null)
                {
                    VirtualMachine newVM = new VirtualMachine
                    {
                        apply_id = edit_record.id,
                        due_time = dueTime,
                        owner_id = edit_record.sender_id,
                        user_id = edit_record.sender_id,
                        vm_name = edit_record.vm_name
                    };
                    context.VirtualMachines.Add(newVM);
                }
                else
                    vm.due_time = dueTime;
