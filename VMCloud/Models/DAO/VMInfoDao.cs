...
namespace VMCloud.Models.DAO
{
    ...
    public class NodeDao
    {
        ...
        public static int GetNumber()
        {
            ...
            {
                List<Node> infos = dbContext.Nodes.ToList();
                List<int> ids = infos.Select(c => c.id).ToList();
                return ids.Count();
            }
        }

        public static void Add(Node info)
        {
            using (var dbContext = new DataModels())
            {
                info = dbContext.Nodes.Add(info);
                dbContext.SaveChanges();
            }
        }

        public static void Update(string host, string name)
        {
            using (var dbContext = new DataModels())
            {
                List<int> id = dbContext.Nodes.Where(u => u.name == name).Select(u => u.id).ToList();
                if (id.Count() == 0)
                    return;
                Node info = dbContext.Nodes.Find(id[0]);
                info.host = host;
                dbContext.SaveChanges();
            }
        }
