        ...
        public static bool Log(HttpRequestMessage request, string target_type, string target_id, string operator_id, int? _operator_role, string _state = "", string _content = "", string _complete_time = "")
        {
            try
            {
                using (var logger = new Logger())
                {
                    ...
                    string user_ip;
                    var realIp = HttpUtil.GetHeader(request, "X-Real-IP").First();
                    if (realIp != null)
                        user_ip = realIp.ToString();
                    else
                    {
                        var forward = HttpUtil.GetHeader(request, "X-Forwarded-For").First();
                        if (forward != null)
                            user_ip = forward.ToString();
                        else
                            user_ip = "";
                    }
                    log.user_ip = user_ip;
